...
const faqData = [
  {
    ...
    answer: 'Yes, EVO-A provides free access for startups, investors, incubators, and viewers. Advanced features and premium services may be added in the future.'
  },
  {
...
function FAQItem({ question, answer, isOpen, onToggle, isDark }) {
  return (
    <div className={`group relative rounded-xl sm:rounded-2xl transition-all duration-500 overflow-hidden ${isDark
      ...
      <button
        onClick={onToggle}
        className={`relative z-10 w-full px-4 sm:px-5 md:px-6 py-3 sm:py-4 md:py-5 text-left flex items-center justify-between transition-all duration-300 rounded-xl sm:rounded-2xl ${isDark
      ...
      >
        ...
        <div className={`flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full transition-all duration-300 ${isDark
      ...
      {isOpen && (
        <div className={`relative z-10 px-4 sm:px-5 md:px-6 pb-4 sm:pb-5 md:pb-6 animate-slide-up ${isDark ? 'text-white/80' : 'text-gray-700'
          }`}>
...
export default function FAQSection({ isVisible, isDark, setRef, SectionTitle, openFAQ, toggleFAQ }) {
  return (
    <section
      ref={setRef('faq')}
      className={`relative mt-4 sm:mt-12 md:mt-16 lg:mt-20 transition-all duration-1000 ease-out ${isVisible['faq'] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}
    >
