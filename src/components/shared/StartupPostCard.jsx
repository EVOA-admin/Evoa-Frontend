 ...
 *  post: {
 *    id, authorId,
 *    startupName, startupLogo, tagline, website, sectors,
 *    imageUrl, timeAgo,
 *    pitchViews, supporters, clickThrough,
 *    investorThoughts: [{ name, avatar }],
 *    isLiked, isSaved, likeCount, commentCount,
 *  }
 */
export default function StartupPostCard({
    ...
    // Website click — open in new tab AND record unique click via API
    const handleWebsiteClick = useCallback((e) => {
        e.stopPropagation();
        if (post.id) {
            postsService.recordWebsiteClick(post.id).catch(() => { /* silent fail */ });
        }
        ...
        <div className={`rounded-3xl overflow-hidden shadow-sm border ${isDark ? "bg-gray-900 border-white/8" : "bg-white border-gray-100"}`}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col leading-tight">
                        <div className="flex items-center gap-1.5">
                            ...
                            {/* Website link right after name with chain-link icon */}
                            {post.website && (
                                <button
            ...
            <div className={`flex items-center justify-around px-4 py-2.5 border-t ${isDark ? "border-white/8" : "border-gray-100"}`}>
                ...
                <ActionBtn
                    ...
                    onPress={onComment}
                    label={post.commentCount > 0 ? fmt(post.commentCount) : null}
                    isDark={isDark}
                />
                <ActionBtn
